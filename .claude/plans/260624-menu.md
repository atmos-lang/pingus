...
## Screen dispatch (in menu)

The menu owns the dispatch loop (it is the persistent stack base,
it does not return an id).
The 5 buttons are spawned once and stay alive; each is a 3-branch
`par` (hover ‖ draw ‖ click) and on an inside-click signals the
menu with `emit @2 :Button [id]`.
Note: `@2` targets the menu task; `:parent` (== `@1`) is the
button's pool, one level too low — the buttons are spawned into a
`tasks(5)` pool, which inserts a hierarchy level.
The loop: `val but = await :Button`, `match but@1 -> await
Fade(menu, Blank, text)`; `:Exit` ends the loop.
`Blank` and the `Fade` transition live in `main.atm`
(see `260625-fade.md`).
Orchestration details: `260625-main.md`.
