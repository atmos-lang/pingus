...
menu with `emit @(:parent) :Button [id]`.
The loop: `val id = (await :Button)@1`, `match id -> await
Blank(text)`; `:Exit` ends the loop.
`Blank` (a placeholder screen) lives here too.
Orchestration details + future transition: `260625-main.md`.
